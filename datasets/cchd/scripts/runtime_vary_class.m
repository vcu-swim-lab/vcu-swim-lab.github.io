function runtime_vary_class()
    dlmwrite('runtime_vary_class.csv', -1);
    for noOfDocs = 450:50:500
        noOfTerms = 500;
        noOfMethods = 200;
        ...
        % the matric back
        array = dlmread('runtime_vary_class.csv');
        if size(array, 2) == 1 && array(1, 1) == -1
            array = [];
        end
        array = [array; [noOfDocs array_k]];
        dlmwrite('runtime_vary_class.csv', array);
    end
